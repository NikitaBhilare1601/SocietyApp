...
const Logs = () => {
  ...
  const filteredLogs = logs
    .filter((log) => {
      ...
      const globalMatch =
        formatDate(log.createdAt || log.created_at || "").toLowerCase().includes(query) ||
                    ...
                    {isLoading ? (
                      ...
                      filteredLogs.map(log => (
                        <tr key={log.id} className="border-b border-border hover:bg-muted/30 transition-colors group">
                          <td className="p-4 text-muted-foreground group-hover:text-foreground transition-colors">{formatDate(log.createdAt || log.created_at || "")}</td>
                          <td className="p-4">
