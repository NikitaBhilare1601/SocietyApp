...
const dbPath = path.join(import.meta.dirname, "society.db");
...
try { db.run("ALTER TABLE members ADD COLUMN roleType TEXT DEFAULT 'Society Member'"); } catch (e) { }
try { db.run("ALTER TABLE members ADD COLUMN status TEXT DEFAULT 'Pending'"); } catch (e) { }
try { db.run("ALTER TABLE members ADD COLUMN documents TEXT"); } catch (e) { }
try { db.run("ALTER TABLE members ADD COLUMN canRead INTEGER DEFAULT 1"); } catch (e) { }
try { db.run("ALTER TABLE members ADD COLUMN canEdit INTEGER DEFAULT 0"); } catch (e) { }
try { db.run("ALTER TABLE members ADD COLUMN canDelete INTEGER DEFAULT 0"); } catch (e) { }

try { db.run("ALTER TABLE users ADD COLUMN mobileNumber TEXT"); } catch (e) { }
try { db.run("ALTER TABLE societies ADD COLUMN whatsappNumber TEXT"); } catch (e) { }
try { db.run("ALTER TABLE logs ADD COLUMN created_at DATETIME DEFAULT CURRENT_TIMESTAMP"); } catch (e) { }
...
try { db.run("ALTER TABLE roles ADD COLUMN whatsappNumber TEXT"); } catch (e) { }
try { db.run("ALTER TABLE roles ADD COLUMN isWhatsApp INTEGER DEFAULT 0"); } catch (e) { }
